...
public class LoadUsersAndRolesQueryHandler : IQueryHandler<LoadUsersAndRolesQuery, LoadUsersAndRolesResponse>
{
    ...
    {
        var roles = (await appRoles.GetAppRoles()).Select(p => new LoadUsersAndRolesResponse.Role(p.Id.GetValueOrDefault(), p.DisplayName));

        var roleAssignments = await appRoleAssignments.GetAppRoleAssignments();

        var users = roleAssignments
            .GroupBy(a => a.PrincipalId)
            .Select(group => new LoadUsersAndRolesResponse.User(
                group.First().PrincipalDisplayName,
                group.Key!.Value,
                group
